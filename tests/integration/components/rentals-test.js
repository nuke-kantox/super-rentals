...
module('Integration | Component | rentals', function (hooks) {
  ...
  test('it renders all given rental properties by default', async function (assert) {
    ...
    assert.dom('.uk-grid').exists();
    assert.dom('.uk-card').exists({ count: 3 });

    assert
      .dom(findAll('.uk-card-title')[0])
      .containsText('Grand Old Mansion');

    assert
      .dom(findAll('.uk-card-title')[1])
      .containsText('Urban Living');

    assert
      .dom(findAll('.uk-card-title')[2])
  ...
  test('it updates the results according to the search query', async function (assert) {
    ...
    await fillIn('input', 'Downtown');

    assert.dom('.uk-grid').exists();
    assert.dom('.uk-card').exists({ count: 1 });
    assert.dom('.uk-card-title').containsText('Downtown Charm');

    await fillIn('input', 'Mansion');

    assert.dom('.uk-grid').exists();
    assert.dom('.uk-card').exists({ count: 1 });
    assert.dom('.uk-card-title').containsText('Grand Old Mansion');
  });
});
